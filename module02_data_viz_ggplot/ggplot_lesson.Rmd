...
The simplified version of the data set can be found in the ``course_data`` folder 
(`burghardt_et_al_2015_expt1.csv`). 

Run the command below to read the data and store it in a variable called ``expt1`` 
(referring to "experiment 1" of the published dataset):
...
plotting to get even more insights. 
...
The `ggplot()` function initialises a plot. And the very minimum it needs 
a dataset to plot:
...
Let's start with `facet_wrap()` and see it in action:
...
Now let's see how the same code works with `facet_grid()`:
...
In this case, our facets are arranged in individual columns. We can arrange them 
in rows like so:
...
![Fig. 2B-C of Burghard et al 2015](http://onlinelibrary.wiley.com/store/10.1111/nph.13799/asset/image_n/nph13799-fig-0002.png?v=1&t=j9vceh13&s=ab8a38f8975413c52f62d9d4ebac20b239bfed4d). **Hint**: facet 
the plot by `day.length` and `temperature` and fill the boxplots by `fluctuation`.

```{r}
ggplot(expt1, aes(genotype, days.to.bolt)) + 
  geom_boxplot(aes(fill = fluctuation)) +
...
to produce "ridge" plots (also known as "joyplots")
