...
using `dplyr`, also part of the `tidyverse` package.
...
Let's say that we wanted to retain only the rows of our table were plants were 
vernalized. This is a job for the `filter()` function:

```{r}
expt1.vernalised <- filter(expt1, vernalization == "V")
head(expt1.vernalised)
```


In order to set the filtering conditions, several operators can be used:
...
3. Samples of genotypes _fca-6_ for which `blade.ratio` is not missing
...
```{r}
expt1.cm <- mutate(expt1, blade.length.cm = blade.length.mm/10 , total.leaf.length.cm = total.leaf.length.mm/10)
```
...
```{r}
expt1.filtered <- mutate(expt1, total.leaf.length.cm = total.leaf.length.mm/10)

expt1.filtered <- filter(expt1.filtered, vernalization=="V")

expt1.filtered <- select (expt1.filtered, genotype, total.leaf.length.cm )
```
...
- Keep samples that are not in the the Ler background and that have been treated 
with fluctuating temperature.
- Transform the blade length in cm
- Keep only the columns containing information about the genotype, blade length 
(in cm) and bolting information (hint, use `contains`)
...
We can pipe data manipulation with `ggplot` in order to produce graphs for a 
subset of the data.
...
Can you make a violin plot of the days to flower for the different genotypes that 
have been growing in short days? 
...
This kind of operation can be achieved by combining two `dplyr` functions together: 
...
write_csv(expt1.blade.ratio.clean, path = "data/x_blade_ratio_clean.csv")
```
